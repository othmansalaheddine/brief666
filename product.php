...
<body class="bg-gray-100 p-8 bg-gray-100">
    ...
    <div class="container mx-auto flex items-center justify-between">
      ...
      <div>
        <a href=""><img style="text-align: center; margin-left:50%" width="10%" height="10%"
            src="./Layout/img/Nouveau_projet1.png" alt=""></a>
        <h1
          style="text-align: center; margin-left:120px;font-size:30px;color:aliceblue;font-family:Cambria, Cochin, Georgia, Times, 'Times New Roman', serif , bold;">
          Electro_Nacer</h1>
      </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 ">
    ...
    <form method="post" action="./product.php">
        ...
        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-2" type="submit" name="filterCategory">Filtrer</button>
    </form>
</div>
</div>
      <div class="mb-5">
    ...
    <form method="post" action="./product.php?action=filterByCat">
        ...
        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-2" type="submit">Filtrer</button>
    </form>
</div>

      <div class="mt-5">
    <form method="post" action="./product.php?action=filterByOrderPrice">
        <label class="font-bold italic text-black ml-5" for="sort">Sort by:</label>
        <select class="w-48 border rounded py-2 px-3" name="sort" id="sort">
...
if (isset($_POST['filterCategory'])) {
    $selectedCategory = $_POST['categorias'];

    switch ($selectedCategory) {
        case 'op-1':
            $priceRange = "BETWEEN 100 AND 200";
            break;
        case 'op-2':
            $priceRange = "BETWEEN 300 AND 400";
            break;
        case 'op-3':
            $priceRange = "> 400";
            break;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['action']) && $_GET['action'] == 'filterByCat') {
    $category = $_POST['category'];
    if ($category == 0) {
      $sql = "SELECT * FROM product";
    } else {
      $sql = "SELECT * FROM product WHERE id_c = $category";
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['action']) && $_GET['action'] == 'filterByOrderPrice') {
    ...
    if ($sort == '') {
      ...
      $sql = "SELECT * FROM product ORDER BY prix $sort";
    }
    ...
    <div>
      <footer class="bg-white lg:grid lg:grid-cols-5">
        <div class="relative block h-32 lg:col-span-2 lg:h-full">
          <img src="./Layout/img/Nouveau_projet1.png" style="background-color:  #000033;" alt=""
